                                ...
                                verbose = getOption("recount3_verbose", TRUE)) 
{
  ...
  if (type %in% c("gene", "exon")) {
    counts <- recount3::read_counts(file_retrieve(url = locate_url(project = project, 
  ...
  stopifnot(`Metadata external_id and counts colnames are not matching.` = identical(metadata$external_id, 
                                                                                     ...
                                                                                     colnames(counts)))
  if (type == "gene") {
    ###### mcgaughey parikh edit ###########
    counts2 <- counts[feature_info$gene_id, ]
    counts <- counts2
    #######################################
    ...
    ###### mcgaughey parikh edit ###########
    counts2 <- counts[feature_info$recount_exon_id, ]
    counts <- counts2
    #######################################
  ...
  colnames(mcols(feature_info))[colnames(mcols(feature_info)) == 
                                  "score"] <- "bp_length"
